...
public class RegistrationController extends AnchorPane
    ...
    private final ObservableList<AccountType> accountTypeList
            ...
            AccountType.Worker, AccountType.Manager);
    ...
    private boolean validator() {
        ...
        if (id.getText() == null || id.getText().length() < 2) {
            ...
            errorMessage.setText("Name fields are required and is restricted "
                    + "to letters only");
            ...
            errorMessage.setText("Email confirmation does not match your "
                    + "email address");
        } else if (password.getText().length() < 6
                || !password.getText().matches("[a-zA-Z0-9]*")) {
            errorMessage.setText("Password must be more than 6 alphanumeric "
                    + "characters");
        } else if (!password.getText().equals(passwordConfirm.getText())) {
            errorMessage.setText("Password confirmation does not match your "
                    + "password");
        } else {
            return true;
        }
     ...
     * @param event the event starts of when the key is touched
     */
    @FXML
